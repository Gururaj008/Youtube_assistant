...
def vdb_from_url(url,openai_api_key):
...
if __name__ == "__main__":
    ...
    st.markdown("""
                    ...
                    <p class="custom-text-01">Youtube assistant using Langchain and OPENAI</p>
                    """, unsafe_allow_html=True)
    st.divider()
    st.markdown('<div style="text-align: justify">The Streamlit application “Youtube Assistant using Langchain and OPENAI” is a sophisticated tool that leverages the power of machine learning and natural language processing to enhance the user’s interaction with YouTube content. The application accepts a YouTube URL and a user question as inputs, and uses similarity search on a vector database containing embeddings to provide relevant answers. This is achieved by transforming the content of the YouTube video into a searchable vector database using Langchain and OpenAI technologies.</div>', unsafe_allow_html=True)
    st.write('')
    st.markdown('<div style="text-align: justify">The application begins by loading the transcript of the provided YouTube video using the YoutubeLoader class. The transcript is then split into manageable chunks using the RecursiveCharacterTextSplitter class, and these chunks are transformed into vector embeddings using the OpenAIEmbeddings class. These embeddings are stored in a FAISS vector database. </div>', unsafe_allow_html=True)
    st.write('')
    ...
    st.write('')
    st.divider()
    open_api_key = st.text_input('OPENAI API Key','Please enter here..',key=1)
    st.write('')
    ...
    if st.button('Fetch the answer..', use_container_width=True):
       db = vdb_from_url(url, open_api_key)
